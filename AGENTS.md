...
This is `platform`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** platform · **Layers:** L4 · **Wave:** 1 · Foundational crates every repo uses: errors, config layering, tracing facade, paths, i18n (fluent), settings schema, diagnostics bundle. Planned components: pub-platform-config · -log · -i18n · -paths · -settings · -diagnose · -secrets-client.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-platform-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-platform-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-platform`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `platform` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
